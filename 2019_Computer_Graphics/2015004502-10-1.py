...
def normalized(v):
    l = l2norm(v)
    if l == 0.: return np.array([0.,0.,0.]) # divide-by-zero warning check
    else: return 1/l * np.array(v)
...
def exp(rv):
    ang = l2norm(rv)
    rv = normalized(rv)
    return np.array([[np.cos(ang)+rv[0]*rv[0]*(1-np.cos(ang)),rv[0]*rv[1]*(1-np.cos(ang))-rv[2]*np.sin(ang),rv[0]*rv[2]*(1-np.cos(ang))+rv[1]*np.sin(ang)],
                     [rv[1]*rv[0]*(1-np.cos(ang))+rv[2]*np.sin(ang),np.cos(ang)+rv[1]*rv[1]*(1-np.cos(ang)),rv[1]*rv[2]*(1-np.cos(ang))-rv[0]*np.sin(ang)],
                     [rv[2]*rv[0]*(1-np.cos(ang))-rv[1]*np.sin(ang),rv[2]*rv[1]*(1-np.cos(ang))+rv[0]*np.sin(ang),np.cos(ang)+rv[2]*rv[2]*(1-np.cos(ang))]])

def log(R):
    ang = np.arccos((R[0][0]+R[1][1]+R[2][2]-1)/2)
    return normalized(np.array([(R[2][1]-R[1][2])/(2*np.sin(ang)),
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,"2015004502-10-1", None,None)
    if not window:
        glfw.terminate()
        return
